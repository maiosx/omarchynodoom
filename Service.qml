...
QtObject {
  ...
  readonly property string backendPath: home + "/.config/omarchy/plugins/nodoom.composer/backend.py"

  property string mode: "intent"
  property string modeLabel: "Browser composer"
  property bool copyDraft: true
  readonly property string actionLabel: "Continue in Nodoom"
  ...
  function safeMessage(value, fallback) {
    ...
    // Backend diagnostics must never turn a handoff URL (and its draft) into UI.
  ...
  function finishStartFailure() {
    ...
    failOperation(operation, "Nodoom composer backend could not start.")
  ...
  function maybeFinishOperation() {
    ...
    if (!response || typeof response !== "object") {
      failOperation(operation, "The Nodoom composer backend returned an invalid response.")
    } else {
  ...
  function failureMessage(kind) {
    ...
    default: return "The Nodoom composer backend operation failed."
    }
  }

  function applyMode(response) {
    mode = "intent"
    copyDraft = response.copyDraft !== false
    modeLabel = safeMessage(response.label, "Browser composer")
  }
  ...
  function handleStatus(response) {
    ...
    if (state === "queued" || state === "running") {
      posting = true
      statusText = safeMessage(response.message, "Opening composer…")
      statusError = false
      return
    }
    ...
    if (state === "posted" || state === "handoff") {
      statusText = state === "posted"
        ? "Posted"
        : "Composer opened with your draft — press Post"
      statusError = false
      ...
      if (state === "posted" && submittedText !== "" && draft === submittedText
